...
datapath = r'C:\Users\brewe\Dropbox\teaching\Courses\BrewerPhDEnv\Homeworks\phdee-2025-db\homework6'
outputpath = r'C:\Users\brewe\Dropbox\teaching\Courses\BrewerPhDEnv\Homeworks\phdee-2025-db\homework6\output'
...
hw6output3 = pd.concat([outputa,outputb,outputc],axis = 1)
hw6output3.columns = ['(a)','(b)','(c)']
hw6output3.index = ['Sedan',' ','MPG','','Constant','']
hw6output3 = pd.concat([hw6output3,pd.DataFrame([[str(nobsa),str(nobsb),str(nobsc)], [fstata,fstatb,fstatc]], index = ['Observations', 'First stage F'], columns = ['(a)', '(b)','(c)'])])


os.chdir(outputpath)
hw6output3.to_latex('hw6output3.tex',column_format = 'lccc', na_rep = ' ')
